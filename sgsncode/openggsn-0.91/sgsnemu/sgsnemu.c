...
int gtp0_c = 0, gtp1_c = 0;
int send_count = 0;

/* receive server info */
int socket_descriptor[MAXCONTEXTS];
struct sockaddr_in sin[MAXCONTEXTS];
int sin_len[MAXCONTEXTS];
struct sockaddr_in r_sin[MAXCONTEXTS];
int r_sin_len[MAXCONTEXTS];
...
int create_pdp_conf(struct pdp_t *pdp, void *cbp, int cause) {
	...
	if (cause < 0) {
		...
		if (iph->pdp->version == 1) {
			retrycount ++;
			if(options.debug) printf("retry:%d\n",retrycount);
	...
	//		inet_ntoa(addr));
	/*socket init*/
	bzero(&sin[contexts_count],sizeof(sin[contexts_count]));
	bzero(&r_sin[contexts_count],sizeof(r_sin[contexts_count]));
	r_sin_len[contexts_count] = sizeof(r_sin);
	sin[contexts_count].sin_family = AF_INET;
	sin[contexts_count].sin_port = htons(PORT);
	sin[contexts_count].sin_addr.s_addr = inet_addr(inet_ntoa(addr));
	sin_len[contexts_count] = sizeof(sin);
	socket_descriptor[contexts_count] = socket(AF_INET, SOCK_DGRAM, 0);
	bind(socket_descriptor[contexts_count], (struct sockaddr *)(sin + contexts_count), sizeof(sin[contexts_count]));
	contexts_count ++;
...
void *count_thread()
{
	...
	{
		sleep(1);
		printf("main gtp:%d,gtp gtp:%d\n", gtp1_c, get_count());
		printf("cotexts_count:%d,send count:%d,receive udp count: %d,sendcount:%d\n", contexts_count, send_count, udp_r_count, udpseq);
		printf("send udp :%d\n", get_send_udp());
	}
}
...
int main(int argc, char **argv)
{
	...
	pthread_t send_pdp_thread_handle, count_thread_handle;
	...
	//send_flag = pthread_create(&send_pdp_thread_handle, NULL, send_pdp_contexts, NULL);
	count_flag = pthread_create(&count_thread_handle, NULL, count_thread, NULL);
	printf("send:%d,count:%d\n",send_flag, count_flag);
	...
	while ((0 != state) && (5 != state)) {
		...
		if ((4 == state) && ((stoptime) <= time(NULL))) {
			printf("timeout after disconnet 4<5:%d\n", time(NULL));
			state = 5;
		}
		...
		if (ntransmitted && options.pingcount && nreceived >= options.pingcount) {
			ping_finish();
			if (!options.createif)
			printf("pint packets are missing3:%d\n", time(NULL));
				state = 3;
		}    

		/* Send off disconnect */
		if (3 == state) {
			printf("send off disconnet 3<4:%d\n", time(NULL));
			state = 4;
		...
		while (( diff<=0 ) && 
			...
			((pingseq < options.pingcount) || (options.pingcount == 0))) {
				...
				while (( diff<=0 ) && 
						...
						((udpseq < options.udpcount) ) && (options.contexts <= contexts_count)) {
					...
					if (diff <=0) {
						//if(contexts_count == options.contexts || udpseq <= contexts_count)
						//	{
						memcpy(&tempsrc, &(iparr[udpseq% contexts_count].pdp->eua.v[2]), 4); /* Copy a 4 byte address */
						if (options.debug) printf("Create_udp ,contexts:%d,%d,udpseq:%d,ip:%s\n",  options.contexts, contexts_count, udpseq, inet_ntoa(tempsrc));
						create_udp(gsn, iparr[udpseq% contexts_count].pdp,
				...
				FD_SET(gsn->fd1c, &fds);
				FD_SET(gsn->fd1u, &fds);
				//FD_SET(socket_descriptor[udpseq% options.contexts], &fds);

				gtp_retranstimeout(gsn, &idleTime);
				ping_timeout(&idleTime);
				if(options.udphost.s_addr)
				{
					if(options.contexts == contexts_count)
					{
				...
				switch (select(maxfd + 1, &fds, NULL, NULL, &idleTime)) {
					case -1:
						printf("options contexts:%d,actual contexts:%d\n",options.contexts, contexts_count);
						sys_err(LOG_ERR, __FILE__, __LINE__, 0,
								"Select returned -1");
						break;  
					case 0:
						if(send_count <= options.contexts)
						{
							send_pdp_contexts(send_count);
							send_count ++;
						}
						//gtp_retran(gsn); /* Only retransmit if nothing else */
						break; 
					default:
						break;
				}
				...
				/*for(n = 0;n < options.contexts;n ++)
				  {
				  printf("3\n");
				  if (FD_ISSET(socket_descriptor[n], &fds))
				  {
				  printf("4,%d\n", n);
				  recvfrom(socket_descriptor[n], message, sizeof(message), 0, (struct sockaddr *)(sin + n), (sin_len + n));
				  printf("5\n");
				//printf("%s\n", message);
				}
				...
				//if(contexts_count != 0)
				//printf("number:%d\n", (udpseq% contexts_count));

				/*	if (FD_ISSET(socket_descriptor[udpseq% options.contexts], &fds))
					{
					if(options.debug) printf("strat receive\n");
					recvfrom(socket_descriptor[udpseq% options.contexts], message, sizeof(message), 0, (struct sockaddr *)(r_sin + (udpseq% options.contexts)), (r_sin_len + (udpseq% options.contexts)));
					printf("%s\n", message);
					}*/
	...
	if(options.udphost.s_addr)
	{
		for(n = 0;n < contexts_count;n ++)
		{
			close(socket_descriptor[n]);
		}
	}
	if (0 == state)
