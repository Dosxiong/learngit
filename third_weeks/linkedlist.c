...
typedef struct node
{
	...
	void *datapointer;           ///datapointer数据指针
}N;
...
void string_sort_xier(N *head,int (*compare)(void *,void *))
{
	...
	if(linkedlist_array != NULL)
	{
		...
		while(increment > 1)
		{
			...
			{
				if(compare(linkedlist_array[i]->datapointer,linkedlist_array[i-increment]->datapointer) < 0)
				{
					temp_pointer = (void *)(linkedlist_array[i]->datapointer); /// 
					for(j = i-increment;j >= 0 &&compare(temp_pointer,linkedlist_array[j]->datapointer) < 0;j = j-increment)
					{
